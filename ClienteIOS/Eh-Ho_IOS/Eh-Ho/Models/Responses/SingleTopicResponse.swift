...
//  Eh-Ho
//
//  Created by Monica Sanmartin on 21/07/2019.
...
struct SingleTopicResponse: Codable {
    ...
    let draftSequence: Int? // Revisar
    
    let posted: Bool? // Revisar
    
    let pinned: Bool
    
    let currentPostNumber: Int? //revisar
    
    let highestPostNumber: Int
    
    let lastReadPostNumber: Int? //revisar
    
    let lastReadPostID: Int? //revisar
...
struct SuggestedTopic: Codable {
   ...
   let unread: Int? // .. REVISAR
    
    let newPosts: Int? // REVISAR
    ...
    let notificationLevel: Int? //REVISAR
    
    let bookmarked: Bool? // REVISAR
    
    let liked: Bool? // REVISAR
...
struct Post: Codable {
    ...
    let actionsSummary: [PostActionsSummary]? //añado ? tras no parsear los post creados desde app
